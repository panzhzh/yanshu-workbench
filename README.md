...
每一轮都会先显式新建独立的空白 Chat，再在该会话中选择推理档位并传入文件，不会修改用户原本打开的聊天。在 Windows 上，YanShu 默认把白名单中的 `.tex`、`.bib`、`.pdf` 与图件作为真实文件复制到系统剪贴板，再粘贴到 ChatGPT；不会把文件内容摊平成一段巨型文本。如果 ChatGPT 已接受准确的档位点击、但新版界面暂时无法回读当前标签，YanShu 会将其记录为 `click-acknowledged` 并继续；只有选项未找到、点击失败、新会话未建立或回读明确冲突时才暂停。
